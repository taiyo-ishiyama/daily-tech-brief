...
function isObjectNode(value, schema) {
  return isObject(value) && typeof value._type == "string" && typeof value._key == "string" && value._type !== schema.block.name && value._type !== schema.span.name;
}
function isLeaf(value, schema) {
  return Text$1.isText(value, schema) || isObjectNode(value, schema);
}
...
function* getNodes(root, schema, options = {}) {
  ...
  for (; !(to && (reverse ? Path.isBefore(p, to) : Path.isAfter(p, to))); ) {
    if (visited.has(n2) || (yield [n2, p]), !visited.has(n2) && !isLeaf(n2, schema) && n2.children.length !== 0 && (pass == null || pass([n2, p]) === !1)) {
      ...
      Path.isAncestor(p, from) && (nextIndex = from[p.length]), p = p.concat(nextIndex), n2 = getNode(root, p, schema);
      continue;
    }
    if (p.length === 0)
      break;
    ...
    if (reverse && p[p.length - 1] !== 0) {
      p = Path.previous(p), n2 = getNode(root, p, schema);
      continue;
    }
    p = Path.parent(p), n2 = getNode(root, p, schema), visited.add(n2);
  }
}
...
function getFirst(root, path2, schema) {
  const p = path2.slice();
  let n2 = getNode(root, p, schema);
  for (; n2 && !isLeaf(n2, schema); ) {
    const ancestor = n2;
    if (ancestor.children.length === 0)
      break;
    n2 = ancestor.children[0], p.push(0);
  }
  return [n2, p];
}
function getLast(root, path2, schema) {
  ...
  for (; n2 && !isLeaf(n2, schema); ) {
    ...
    n2 = ancestor.children[i], p.push(i);
  }
  return [n2, p];
}
...
function normalize(editor, options = {}) {
  ...
  if (isNormalizing(editor)) {
    if (force) {
      const allPaths = Array.from(getNodes(editor, editor.schema), ([, p]) => p), allPathKeys = new Set(allPaths.map((p) => p.join(",")));
    ...
    getDirtyPaths2(editor).length !== 0 && withoutNormalizing(editor, () => {
      for (const dirtyPath of getDirtyPaths2(editor))
        if (hasNode(editor, dirtyPath, editor.schema)) {
          ...
          Element$2.isElement(entryNode, editor.schema) && entryNode.children.length === 0 && editor.normalizeNode(entry, {
            operation
          });
        }
...
function* getTexts(root, schema, options = {}) {
  for (const [node2, path2] of getNodes(root, schema, options))
    Text$1.isText(node2, schema) && (yield [node2, path2]);
}
const Editor = {
...
}, insertChildren = (xs, index, ...newValues) => [...xs.slice(0, index), ...newValues, ...xs.slice(index)], replaceChildren = (xs, index, removeCount, ...newValues) => [...xs.slice(0, index), ...newValues, ...xs.slice(index + removeCount)], removeChildren = replaceChildren, modifyDescendant = (root, path2, schema, f) => {
  if (path2.length === 0)
    throw new Error("Cannot modify the editor");
  const node2 = getNode(root, path2, schema), slicedPath = path2.slice();
  ...
  path2.length === 0 ? root.children = f(root.children) : modifyDescendant(root, path2, schema, (node2) => {
    if (Text$1.isText(node2, schema) || isObjectNode(node2, schema))
  ...
  switch (op.type) {
    case "insert_node": {
      ...
      modifyChildren(editor, Path.parent(path2), editor.schema, (children) => {
        ...
        return insertChildren(children, index, node2);
    ...
    case "insert_text": {
      ...
      if (text.length === 0)
        break;
      modifyLeaf(editor, path2, editor.schema, (node2) => {
    ...
    case "remove_node": {
      ...
      if (modifyChildren(editor, Path.parent(path2), editor.schema, (children) => removeChildren(children, index, 1)), editor.selection) {
        let selection = {
          ...editor.selection
        };
        for (const [point2, key] of Range.points(selection)) {
          ...
          if (selection != null && result != null)
            ...
            let prev, next2;
            for (const [n2, p] of getTexts(editor, editor.schema))
  ...
  switch (op.type) {
    ...
    case "insert_node": {
      const {
        node: node2,
        path: path2
      } = op, levels2 = Path.levels(path2), descendants = Text$1.isText(node2, _editor.schema) ? [] : Array.from(getNodes(node2, _editor.schema), ([, p]) => path2.concat(p));
...
function applyMergeNode(editor, path2, position) {
  const node2 = getNode(editor, path2, editor.schema), prevPath = Path.previous(path2);
  for (const ref of editor.pathRefs) {
...
const getDefaultInsertLocation = (editor) => editor.selection ? editor.selection : editor.children.length > 0 ? end(editor, []) : [0], matchPath = (editor, path2) => {
  const [matchedNode] = node(editor, path2);
  return (n2) => n2 === matchedNode;
...
}, EDITOR_BRAND = /* @__PURE__ */ Symbol.for("slate-editor");
...
function point(editor, at, options = {}) {
  ...
  if (Path.isPath(at)) {
    ...
    const node2 = getNode(editor, path2, editor.schema);
    if (!Text$1.isText(node2, editor.schema) && !Element$2.isElement(node2, editor.schema) && !isEditor(node2))
      return {
        path: path2,
        offset: 0
      };
...
function getVoid(editor, options = {}) {
  const {
    at = editor.selection
  } = options;
  if (!at)
  ...
  return editor.isObjectNode(node2) ? [node2, nodePath] : above(editor, {
    ...options,
    match: (n2) => editor.isObjectNode(n2)
  });
}
...
function isEnd(editor, point2, at) {
  ...
  return Point.equals(point2, editorEnd);
}
...
function isStart(editor, point2, at) {
  ...
  return Point.equals(point2, editorStart);
}
...
function* nodes(editor, options = {}) {
  ...
  const nodeEntries = getNodes(editor, editor.schema, {
    ...
    from,
    to,
    pass: ([node2, path2]) => pass && pass([node2, path2]) ? !0 : Element$2.isElement(node2, editor.schema) ? !!(!voids && Editor.isElementReadOnly(editor, node2)) : !1
  }), matches = [];
  let hit;
  for (const [node2, path2] of nodeEntries) {
    ...
    if (!match2(node2, path2)) {
      if (universal && !isLower && Text$1.isText(node2, editor.schema))
        return;
      continue;
    }
...
function unhangRange(editor, range2, options = {}) {
  ...
  let [start$1, end2] = Range.edges(range2);
  if (start$1.offset !== 0 || end2.offset !== 0 || Range.isCollapsed(range2) || Path.hasPrevious(end2.path))
    return range2;
  const endBlock = above(editor, {
    at: end2,
    match: (n2) => Element$2.isElement(n2, editor.schema) && isBlock(editor, n2)
...
function extractProps(node2, schema) {
  if (Element$2.isAncestor(node2, schema)) {
    ...
    return properties;
  } else if (Text$1.isText(node2, schema)) {
    const {
      text: _text,
      ...properties
    } = node2;
...
function isNode(value, schema) {
  return Text$1.isText(value, schema) || Element$2.isElement(value, schema) || isEditor(value) || isObjectNode(value, schema);
}
...
function hasInlines(editor, element) {
  return element.children.some((n2) => Text$1.isText(n2, editor.schema) || Editor.isInline(editor, n2));
}
...
function range(editor, at, to) {
  if (Range.isRange(at) && !to)
    return at;
  const rangeStart = start(editor, at), rangeEnd = end(editor, to || at);
  return {
    anchor: rangeStart,
    focus: rangeEnd
  };
...
function string(editor, at, options = {}) {
  const {
    voids = !1
  } = options, editorRange = range(editor, at), [start2, end2] = Range.edges(editorRange);
  let text = "";
  for (const [node2, path2] of nodes(editor, {
    at: editorRange,
    match: (n2) => Text$1.isText(n2, editor.schema),
...
function* positions(editor, options = {}) {
  ...
  if (!at)
    return;
  const editorRange = range(editor, at), [start$1, end$1] = Range.edges(editorRange), first = reverse ? end$1 : start$1;
  ...
  for (const [node2, nodePath] of nodes(editor, {
    ...
    const hasSkippedAncestor = skippedPaths.some((p) => Path.isAncestor(p, nodePath));
    ...
    if (Element$2.isElement(node2, editor.schema)) {
      if (!editor.isSelectable(node2))
        if (skippedPaths.push(nodePath), reverse) {
          Path.hasPrevious(nodePath) && (yield* maybeYield(end(editor, Path.previous(nodePath))));
          continue;
        } else {
          const nextPath = Path.next(nodePath);
          hasPath(editor, nextPath) && (yield* maybeYield(start(editor, nextPath)));
          continue;
        }
      ...
      if (hasInlines(editor, node2)) {
        const e = Path.isAncestor(nodePath, end$1.path) ? end$1 : end(editor, nodePath), s = Path.isAncestor(nodePath, start$1.path) ? start$1 : start(editor, nodePath);
    ...
    if (!isEditor(node2) && !Element$2.isElement(node2, editor.schema) && !Text$1.isText(node2, editor.schema)) {
      yield* maybeYield({
        path: nodePath,
        offset: 0
      });
    ...
    if (Text$1.isText(node2, editor.schema)) {
      ...
      for (isFirst ? (leafTextRemaining = reverse ? first.offset : node2.text.length - first.offset, leafTextOffset = first.offset) : (leafTextRemaining = node2.text.length, leafTextOffset = reverse ? leafTextRemaining : 0), (isFirst || isNewBlock || unit === "offset") && (yield* maybeYield({
        path: nodePath,
...
function elementReadOnly(editor, options = {}) {
  return above(editor, {
    ...options,
    match: (n2) => Element$2.isElement(n2, editor.schema) && Editor.isElementReadOnly(editor, n2)
  });
}
function parent(editor, at, options = {}) {
  const nodePath = path(editor, at, options), parentPath = Path.parent(nodePath);
  return node(editor, parentPath);
}
function previous(editor, options = {}) {
  ...
  if (!pointBeforeLocation)
    return;
  const [, to] = node(editor, path(editor, [], {
    edge: "start"
...
function shouldMergeNodesRemovePrevNode(_editor, [prevNode, prevPath], [_curNode, _curNodePath]) {
  return Element$2.isElement(prevNode, _editor.schema) && (prevNode.children.length === 0 || prevNode.children.length === 1 && Text$1.isText(prevNode.children[0], _editor.schema) && prevNode.children[0].text === "") || Text$1.isText(prevNode, _editor.schema) && prevNode.text === "" && prevPath[prevPath.length - 1] !== 0;
}
function insertText(editor, text, options = {}) {
  withoutNormalizing(editor, () => {
    ...
    if (Path.isPath(at) && (at = range(editor, at)), Range.isRange(at))
      if (Range.isCollapsed(at))
        at = at.anchor;
      else {
        const end2 = Range.end(at);
        if (!voids && getVoid(editor, {
          at: end2
        }))
          return;
        const start2 = Range.start(at), startRef = pointRef(editor, start2), endRef = pointRef(editor, end2);
        deleteText(editor, {
          at,
          voids
        });
...
function removeNodes(editor, options = {}) {
  withoutNormalizing(editor, () => {
    ...
    if (!at)
      return;
    match2 == null && (match2 = Path.isPath(at) ? matchPath(editor, at) : (n2) => Element$2.isElement(n2, editor.schema) && isBlock(editor, n2)), !hanging && Range.isRange(at) && (at = unhangRange(editor, at, {
      voids
    }));
    ...
    for (const ref of pathRefs) {
      ...
      if (path2) {
        const [removedNode] = node(editor, path2);
        editor.apply({
          type: "remove_node",
          path: path2,
          node: removedNode
...
function deleteText(editor, options = {}) {
  withoutNormalizing(editor, () => {
    ...
    if (Range.isRange(at) && Range.isCollapsed(at) && (isCollapsed = !0, at = at.anchor), Point.isPoint(at)) {
      const furthestVoid = getVoid(editor, {
        at,
        mode: "highest"
      });
    ...
    if (Range.isCollapsed(at))
      return;
    if (!hanging) {
      const [, end2] = Range.edges(at), endOfDoc = end(editor, []);
      Point.equals(end2, endOfDoc) || (at = unhangRange(editor, at, {
        voids
      }));
    }
    let [start$1, end$1] = Range.edges(at);
    const startBlock = above(editor, {
      ...
      at: end$1,
      voids
    }), isAcrossBlocks = startBlock && endBlock && !Path.equals(startBlock[1], endBlock[1]), isSingleText = Path.equals(start$1.path, end$1.path), startNonEditable = voids ? null : getVoid(editor, {
      at: start$1,
      mode: "highest"
    }) ?? elementReadOnly(editor, {
      at: start$1,
      mode: "highest"
    }), endNonEditable = voids ? null : getVoid(editor, {
      at: end$1,
      mode: "highest"
    }) ?? elementReadOnly(editor, {
      at: end$1,
    ...
    if (startNonEditable) {
      ...
      beforePoint && startBlock && Path.isAncestor(startBlock[1], beforePoint.path) && (start$1 = beforePoint);
    }
    if (endNonEditable) {
      const afterPoint = after(editor, end$1);
      afterPoint && endBlock && Path.isAncestor(endBlock[1], afterPoint.path) && (end$1 = afterPoint);
    }
    const matches = [];
    let lastPath;
    for (const entry of nodes(editor, {
      ...
      lastPath && Path.compare(path2, lastPath) === 0 || (!voids && (editor.isObjectNode(node2) || Element$2.isElement(node2, editor.schema) && Editor.isElementReadOnly(editor, node2)) || !Path.isCommon(path2, start$1.path) && !Path.isCommon(path2, end$1.path)) && (matches.push(entry), lastPath = path2);
    }
    ...
    if (!isSingleText && !startNonEditable) {
      ...
      if (Text$1.isText(node2, editor.schema)) {
        const {
          ...
          offset
        } = start$1, text = node2.text.slice(offset);
        text.length > 0 && (editor.apply({
          type: "remove_text",
          path: path2,
          offset,
    ...
    if (!isSingleText && isAcrossBlocks && endRef.current && startRef.current) {
      const mergeAt = endRef.current, mergeMatch = (n2) => Element$2.isElement(n2, editor.schema) && isBlock(editor, n2), [current] = nodes(editor, {
        at: mergeAt,
        match: mergeMatch,
        voids,
        mode: "lowest"
      ...
      if (current && prev) {
        ...
        if (mergePath.length !== 0 && prevPath.length !== 0) {
          const newPath = Path.next(prevPath), commonPath = Path.common(mergePath, prevPath), isPreviousSibling = Path.isSibling(mergePath, prevPath), editorLevels = Array.from(levels(editor, {
            at: mergePath
...
function insertNodes(editor, nodes$1, options = {}) {
  withoutNormalizing(editor, () => {
    const {
      hanging = !1,
      voids = !1,
    ...
    if (isNode(nodes$1, editor.schema) && (nodes$1 = [nodes$1]), nodes$1.length === 0)
      return;
    ...
    if (at || (at = getDefaultInsertLocation(editor), select2 !== !1 && (select2 = !0)), select2 == null && (select2 = !1), Range.isRange(at))
      if (hanging || (at = unhangRange(editor, at, {
        ...
        const [, end2] = Range.edges(at), endPointRef = pointRef(editor, end2);
        deleteText(editor, {
          at
        }), at = endPointRef.unref();
      }
    if (Point.isPoint(at)) {
      match2 == null && (Text$1.isText(node2, editor.schema) ? match2 = (n2) => Text$1.isText(n2, editor.schema) : editor.isInline(node2) ? match2 = (n2) => Text$1.isText(n2, editor.schema) || Editor.isInline(editor, n2) : match2 = (n2) => Element$2.isElement(n2, editor.schema) && isBlock(editor, n2));
          ...
          try {
            ...
            if (highest) {
              ...
              for (const [node22, nodePath] of levels(editor, {
                ...
                if (!isEdge(editor, point2, nodePath)) {
                  split = !0;
                  const properties = extractProps(node22, editor.schema);
      ...
      if (batchDirty) {
        ...
        batchDirtyPaths(editor, () => {
          for (const node22 of nodes$1) {
            ...
            editor.apply(op), at = Path.next(at), batchedOps.push(op), Text$1.isText(node22, editor.schema) ? newDirtyPaths.push(path2) : newDirtyPaths.push(...Array.from(getNodes(node22, editor.schema), ([, p]) => path2.concat(p)));
          }
        }, () => {
...
const normalizeNode = (editor, entry) => {
  ...
  if (Text$1.isText(node2, editor.schema) || editor.isObjectNode(node2))
    return;
  "children" in node2 || (node2.children = []);
  let element = node2;
  if (element !== editor && element.children.length === 0) {
    ...
    insertNodes(editor, child, {
      at: path2.concat(0),
      voids: !0
    }), element = getNode(editor, path2, editor.schema);
  }
  ...
  if (element !== editor && (editor.isInline(element) || Text$1.isText(firstChild, editor.schema) || editor.isObjectNode(firstChild) || Element$2.isElement(firstChild, editor.schema) && editor.isInline(firstChild)))
        ...
        if (prev != null && Text$1.isText(prev, editor.schema)) {
          if (child.text === "")
            removeNodes(editor, {
              at: path2.concat(n2),
              voids: !0
            }), element = getNode(editor, path2, editor.schema), n2--;
          else if (prev.text === "")
            removeNodes(editor, {
            ...
            applyMergeNode(editor, mergePath, prev.text.length), element = getNode(editor, path2, editor.schema), n2--;
          }
        }
    ...
    for (let n2 = 0; n2 < element.children.length; n2++) {
      ...
      (Text$1.isText(child, editor.schema) || Element$2.isElement(child, editor.schema) && editor.isInline(child)) && (removeNodes(editor, {
        at: path2.concat(n2),
        voids: !0
      }), element = getNode(editor, path2, editor.schema), n2--);
    }
...
function setSelection$1(editor, props) {
  const {
    selection
...
const DOMEditor = {
  ...
  toSlateNode: (editor, domNode) => {
    ...
    const node2 = domEl ? editor.elementToNode.get(domEl) : null;
    if (!node2)
...
`)) && offset--;
    ...
    if (!textNode && parentNode) {
      if (nearestNode instanceof HTMLElement && nearestNode.hasAttribute("data-slate-node")) {
        ...
        if (childEl instanceof HTMLElement && DOMEditor.hasDOMNode(editor, childEl)) {
          ...
          if (editor.isObjectNode(slateNode2))
            try {
              return {
    ...
    let path2;
    try {
      path2 = DOMEditor.findPath(editor, slateNode);
    } catch (e) {
      if (suppressThrow)
        return null;
      throw e;
...
`) && focusOffset === focusNode.textContent.length && focusOffset--;
    ...
    return Range.isExpanded(range2) && Range.isForward(range2) && isDOMElement(focusNode) && getVoid(editor, {
      at: range2.focus,
      mode: "highest"
...
function verifyDiffState(editor, textDiff) {
  ...
  return Text$1.isText(nextNode, editor.schema) && nextNode.text.startsWith(diff2.text);
}
...
function normalizePoint$1(editor, point2) {
  ...
  if (!Text$1.isText(leaf2, editor.schema))
    return null;
...
const withDOM = (editor) => {
  ...
  return e.isNodeMapDirty = !1, e.domWindow = null, e.domElement = null, e.domPlaceholder = "", e.domPlaceholderElement = null, e.keyToElement = /* @__PURE__ */ new WeakMap(), e.nodeToIndex = /* @__PURE__ */ new WeakMap(), e.nodeToParent = /* @__PURE__ */ new WeakMap(), e.elementToNode = /* @__PURE__ */ new WeakMap(), e.nodeToElement = /* @__PURE__ */ new WeakMap(), e.nodeToKey = /* @__PURE__ */ new WeakMap(), e.readOnly = !1, e.focused = !1, e.composing = !1, e.userSelection = null, e.onContextChange = null, e.scheduleFlush = null, e.pendingInsertionMarks = null, e.userMarks = null, e.pendingDiffs = [], e.pendingAction = null, e.pendingSelection = null, e.forceRender = null, e.apply = (op) => {
    ...
    pendingSelection && (e.pendingSelection = transformPendingRange(e, pendingSelection, op));
    ...
    switch (op.type) {
      ...
      case "set_selection": {
        e.userSelection?.unref(), e.userSelection = null;
        break;
      }
      case "insert_node":
      case "remove_node": {
  ...
  return matches;
}, TRIPLE_CLICK = 3, bold = createKeyboardShortcut({
  default: [{
    key: "B",
    ctrl: !0,
    meta: !1,
...
const PLACEHOLDER_SYMBOL = /* @__PURE__ */ Symbol("placeholder"), MARK_PLACEHOLDER_SYMBOL = /* @__PURE__ */ Symbol("mark-placeholder"), shallowCompare = (obj1, obj2) => Object.keys(obj1).length === Object.keys(obj2).length && Object.keys(obj1).every((key) => obj2.hasOwnProperty(key) && obj1[key] === obj2[key]), isDecorationFlagsEqual = (range2, other) => {
  const {
    anchor: _rangeAnchor,
    focus: _rangeFocus,
    ...rangeOwnProps
  ...
  const path2 = DOMEditor.findPath(editor, node2), level = path2.length, ancestorRange = range(editor, path2), cachedChildRanges = new Array(node2.children.length), getChildRange = (index) => {
    const cachedRange = cachedChildRanges[index];
    if (cachedRange)
      return cachedRange;
    const childRange = range(editor, [...path2, index]);
...
function getChild(root, index, schema) {
  if (Text$1.isText(root, schema) || isObjectNode(root, schema))
...
function getFocusBlock({
  ...
  try {
    return node(editor, editor.selection.focus.path.slice(0, 1)) ?? [void 0, void 0];
  } catch {
    return [void 0, void 0];
  }
}
...
function getPointBlock({
  ...
  try {
    const [block] = node(editor, point2.path.slice(0, 1)) ?? [void 0, void 0];
...
function getFirstBlock({
  ...
  if (editor.children.length === 0)
    return [void 0, void 0];
  const firstBlockPath = start(editor, []).path.at(0);
  try {
    return firstBlockPath !== void 0 ? node(editor, [firstBlockPath]) ?? [void 0, void 0] : [void 0, void 0];
  } catch {
    return [void 0, void 0];
  }
}
function getLastBlock({
  ...
  if (editor.children.length === 0)
    return [void 0, void 0];
  const lastBlockPath = end(editor, []).path.at(0);
  try {
    return lastBlockPath !== void 0 ? node(editor, [lastBlockPath]) ?? [void 0, void 0] : [void 0, void 0];
  } catch {
    return [void 0, void 0];
  }
}
function getNodeBlock({
  ...
  const parent2 = Array.from(nodes(editor, {
    mode: "highest",
    at: [],
  ...
  return Element$2.isElement(parent2, editor.schema) ? elementToBlock({
    element: parent2
  }) : void 0;
}
...
function isBlockElement({
  ...
  return Element$2.isElement(node2, editor.schema) && !editor.isInline(node2) && (schema.block.name === node2._type || schema.blockObjects.some((blockObject) => blockObject.name === node2._type));
}
...
function getEventPosition({
  ...
  if (eventBlock && eventPositionBlock && !eventSelection && !isEditor(eventNode))
    return {
...
function toSlateSelectionPoint(snapshot, selectionPoint, direction) {
  ...
  for (const child of block.children)
    if (childIndex++, child._key === childKey) {
      pathChild = child, isSpan(snapshot.context, child) ? childPath = [childIndex] : (childPath = [childIndex], offset = 0);
      break;
    }
...
function getString(node2, schema) {
  return isObjectNode(node2, schema) ? "" : Text$1.isText(node2, schema) ? node2.text : node2.children.map((child) => getString(child, schema)).join("");
}
...
function leaf(editor, at, options = {}) {
  const leafPath = path(editor, at, options);
  return [getLeaf(editor, leafPath, editor.schema), leafPath];
}
const ReactEditor = DOMEditor, RESOLVE_DELAY = 25, FLUSH_DELAY = 200, debug = (..._) => {
...
function createAndroidInputManager({
  ...
  const applyPendingSelection = () => {
    ...
    if (editor.pendingSelection = null, pendingSelection) {
      ...
      normalized && (!selection || !Range.equals(normalized, selection)) && editor.select(normalized);
    }
  }, performAction = () => {
    ...
    if (editor.pendingAction = null, !!action) {
      if (action.at) {
        ...
        (!editor.selection || !Range.equals(editor.selection, targetRange2)) && editor.select(target);
      }
      action.run();
    }
  }, flush = () => {
    ...
    for (; diff2 = editor.pendingDiffs?.[0]; ) {
      ...
      pendingMarks !== void 0 && (editor.pendingInsertionMarks = null, editor.marks = pendingMarks), pendingMarks && insertPositionHint === !1 && (insertPositionHint = null, debug("insert after mark placeholder"));
      const range2 = targetRange(diff2);
      (!editor.selection || !Range.equals(editor.selection, range2)) && editor.select(range2), diff2.diff.text ? editorActor.send({
        type: "behavior event",
        behaviorEvent: {
          type: "insert.text",
        ...
        id
      }) => id !== diff2.id), verifyDiffState(editor, diff2) || (scheduleSelectionChange = !1, editor.pendingAction = null, editor.userMarks = null, flushing = "action", editor.pendingSelection = null, scheduleOnDOMSelectionChange.cancel(), onDOMSelectionChange.cancel(), selectionRef?.unref());
    }
    const selection = selectionRef?.unref();
    if (selection && !editor.pendingSelection && (!editor.selection || !Range.equals(selection, editor.selection)) && editor.select(selection), hasPendingAction()) {
      performAction();
      return;
    }
    ...
    editor.userMarks = null, userMarks !== void 0 && (editor.marks = userMarks, editor.onChange());
    ...
    if (!Text$1.isText(target, editor.schema))
      return;
    const idx = pendingDiffs.findIndex((change) => Path.equals(change.path, path2));
    if (idx < 0) {
    ...
    if (type.startsWith("delete")) {
      ...
      let [start2, end2] = Range.edges(targetRange2), [leaf$1, path2] = leaf(editor, start2.path);
      if (!Text$1.isText(leaf$1, editor.schema))
      ...
      if (Range.isExpanded(targetRange2) && leaf$1.text.length === start2.offset && end2.offset === 0) {
        const next$1 = next(editor, {
          at: start2.path,
          match: (n2) => Text$1.isText(n2, editor.schema)
        });
        next$1 && Path.equals(next$1[1], end2.path) && (direction === "backward" ? (targetRange2 = {
          anchor: end2,
          focus: end2
        }, start2 = end2, [leaf$1, path2] = next$1) : (targetRange2 = {
          anchor: start2,
          focus: start2
        }, end2 = start2));
      }
      ...
      }, relevantPendingDiffs = editor.pendingDiffs?.find((change) => Path.equals(change.path, path2)), diffs = relevantPendingDiffs ? [relevantPendingDiffs.diff, diff2] : [diff2];
      if (applyStringDiff(leaf$1.text, ...diffs).length === 0 && (canStoreDiff = !1), Range.isExpanded(targetRange2)) {
    ...
    switch (type) {
      ...
      case "deleteContentForward": {
        ...
        if (canStoreDiff && Range.isCollapsed(targetRange2)) {
          ...
          if (Text$1.isText(targetNode, editor.schema) && anchor.offset < targetNode.text.length)
      ...
      case "insertText": {
        ...
        let text = data ?? "";
        if (editor.pendingInsertionMarks && (text = text.replace("\uFEFF", "")), type === "insertText" && /.*\n.*\n$/.test(text) && (text = text.slice(0, -1)), text.includes(`
`))
          return scheduleAction(() => {
...
const DecorateContext = createContext({}), useDecorations = (node2, parentDecorations) => {
  ...
  }, equalityFn = Text$1.isText(node2, editor.schema) ? isTextDecorationsEqual : isElementDecorationsEqual, [decorations, update] = useGenericSelector(selector, equalityFn);
...
const defaultRenderElement$1 = (props) => /* @__PURE__ */ jsx(DefaultElement, { ...props }), Element$1 = (props) => {
  const {
    ...
    ref2 ? (editor.keyToElement?.set(key, ref2), editor.nodeToElement.set(element, ref2), editor.elementToNode.set(ref2, element)) : (editor.keyToElement?.delete(key), editor.nodeToElement.delete(element));
  ...
  if (isInline && (attributes["data-slate-inline"] = !0), !isInline && hasInlines(editor, element)) {
  ...
  return /* @__PURE__ */ jsx("div", { ...attributes, style: {
    position: "relative"
  }, children });
}, ObjectNodeComponent = (props) => {
  const {
    objectNode,
    isInline,
    renderElement = defaultRenderElement
  } = props, editor = useSlateStatic(), readOnly = useReadOnly(), key = ReactEditor.findKey(editor, objectNode), ref = useCallback((ref2) => {
    ref2 ? (editor.keyToElement?.set(key, ref2), editor.nodeToElement.set(objectNode, ref2), editor.elementToNode.set(ref2, objectNode)) : (editor.keyToElement?.delete(key), editor.nodeToElement.delete(objectNode));
  }, [editor, key, objectNode]), syntheticText = useMemo(() => ({
    text: "",
    marks: []
  }), []);
  editor.nodeToIndex.set(syntheticText, 0), editor.nodeToParent.set(syntheticText, objectNode);
  const spacerTextRef = useCallback((span) => {
    span ? (editor.keyToElement?.set(ReactEditor.findKey(editor, syntheticText), span), editor.nodeToElement.set(syntheticText, span), editor.elementToNode.set(span, syntheticText)) : editor.nodeToElement.delete(syntheticText);
  ...
  return isInline && (attributes["data-slate-inline"] = !0, readOnly || (attributes.contentEditable = !1)), renderElement({
    attributes,
    children: /* @__PURE__ */ jsx(isInline ? "span" : "div", { "data-slate-spacer": !0, style: {
      height: "0",
      color: "transparent",
      outline: "none",
      position: "absolute"
    }, children: /* @__PURE__ */ jsx("span", { "data-slate-node": "text", ref: spacerTextRef, children: /* @__PURE__ */ jsx("span", { "data-slate-leaf": !0, children: /* @__PURE__ */ jsx("span", { "data-slate-zero-width": "z", "data-slate-length": 0, children: IS_ANDROID ? null : "\uFEFF" }) }) }) }),
    element: objectNode
  });
}, MemoizedObjectNode = React.memo(ObjectNodeComponent, (prev, next2) => prev.objectNode === next2.objectNode && prev.renderElement === next2.renderElement && prev.isInline === next2.isInline && isElementDecorationsEqual(prev.decorations, next2.decorations)), SlateString = (props) => {
  const {
    isLast,
    leaf: leaf2,
    parent: parent2,
  ...
  return leaf2.text === "" && parent2.children[parent2.children.length - 1] === text && !editor.isInline(parent2) && string(editor, parentPath) === "" ? /* @__PURE__ */ jsx(ZeroWidthString, { isLineBreak: !0, isMarkPlaceholder }) : leaf2.text === "" ? /* @__PURE__ */ jsx(ZeroWidthString, { isMarkPlaceholder }) : isLast && leaf2.text.slice(-1) === `
...
const defaultRenderLeaf = (props) => /* @__PURE__ */ jsx(DefaultLeaf, { ...props }), Leaf = (props) => {
  const {
    ...
    if (disconnectPlaceholderResizeObserver(placeholderResizeObserver, placeholderEl == null), placeholderEl == null)
      editor.domPlaceholderElement = null, leaf2.onPlaceholderResize?.(null);
    else {
  ...
  const attributes = {
    ...
    ref: useCallback((span) => {
      span ? (editor.keyToElement?.set(key, span), editor.nodeToElement.set(text, span), editor.elementToNode.set(span, text)) : (editor.keyToElement?.delete(key), editor.nodeToElement.delete(text), ref.current && editor.elementToNode.delete(ref.current)), ref.current = span;
    }, [ref, editor, key, text])
  };
  return renderText({
    text,
  ...
  return node2.children.map((n2, i) => Element$2.isElement(n2, editor.schema) ? renderElementComponent(n2, i) : isObjectNode(n2, editor.schema) ? renderObjectNodeComponent(n2, i) : renderTextComponent(n2, i));
...
const RestoreDOM = IS_ANDROID ? RestoreDOMComponent : ({
  ...
  const androidInputManagerRef = useRef(void 0), onDOMSelectionChange = useMemo(() => throttle(() => {
    ...
    if ((IS_ANDROID || !ReactEditor.isComposing(editor)) && (!state.isUpdatingSelection || androidInputManager?.isFlushing())) {
      ...
      if (anchorNodeSelectable && focusNodeInEditor) {
        ...
        range2 && (!ReactEditor.isComposing(editor) && !androidInputManager?.hasPendingChanges() && !androidInputManager?.isFlushing() ? editor.select(range2) : androidInputManager?.handleUserSelect(range2));
      }
  ...
  androidInputManagerRef.current = useAndroidInputManager({
    ...
    let window2 = null;
    ref.current && (window2 = getDefaultView(ref.current)) ? (editor.domWindow = window2, editor.domElement = ref.current, editor.nodeToElement.set(editor, ref.current), editor.elementToNode.set(ref.current, editor)) : editor.nodeToElement.delete(editor);
    const {
      selection
  ...
  const onDOMBeforeInput = useCallback((event) => {
    ...
    if (onUserInput(), !readOnly && ReactEditor.hasEditableTarget(editor, event.target) && !isDOMEventHandled(event, propsOnDOMBeforeInput)) {
      ...
      selection.anchor.offset !== 0 && (native = !0, editor.marks && (native = !1), !editor.isNodeMapDirty)) {
        const {
          anchor
      ...
      switch (type) {
        ...
        case "insertReplacementText":
        case "insertText": {
          type === "insertFromComposition" && ReactEditor.isComposing(editor) && (setIsComposing(!1), editor.composing = !1), data?.constructor.name === "DataTransfer" ? editorActor.send({
            type: "behavior event",
            behaviorEvent: {
              type: "input.*",
              originEvent: {
      ...
      editor.userSelection = null, toRestore && (!editor.selection || !Range.equals(editor.selection, toRestore)) && editor.select(toRestore);
    }
  ...
  const decorations = decorate([editor, []]), decorateContext = useDecorateContext(decorate), showPlaceholder = placeholder && editor.children.length === 1 && Array.from(getTexts(editor, editor.schema)).length === 1 && getString(editor, editor.schema) === "" && !isComposing, placeHolderResizeHandler = useCallback((placeholderEl) => {
    setPlaceholderHeight(placeholderEl && showPlaceholder ? placeholderEl.getBoundingClientRect()?.height : void 0);
  }, [showPlaceholder]);
  if (showPlaceholder) {
    const placeholderStart = start(editor, []);
    decorations.push({
    ...
    Component2,
    {
      ...
      onBlur: useCallback((event) => {
        ...
        if (relatedTarget !== el && !(isDOMElement(relatedTarget) && relatedTarget.hasAttribute("data-slate-spacer"))) {
          if (relatedTarget != null && isDOMNode(relatedTarget) && ReactEditor.hasDOMNode(editor, relatedTarget)) {
            ...
            if (Element$2.isElement(node2, editor.schema) || editor.isObjectNode(node2))
              return;
          }
      ...
      onClick: useCallback((event) => {
        if (ReactEditor.hasTarget(editor, event.target) && !isEventHandled(event, attributes.onClick) && isDOMNode(event.target)) {
          ...
          if (!hasPath(editor, path2) || getNode(editor, path2, editor.schema) !== node2)
            return;
          if (event.detail === TRIPLE_CLICK && path2.length >= 1) {
            let blockPath = path2;
            Element$2.isElement(node2, editor.schema) && isBlock(editor, node2) || (blockPath = above(editor, {
              match: (n2) => Element$2.isElement(n2, editor.schema) && isBlock(editor, n2),
              at: path2
            })?.[1] ?? path2.slice(0, 1));
          ...
          if (readOnly)
            return;
          const start$1 = start(editor, path2), end$1 = end(editor, path2), startVoid = getVoid(editor, {
            at: start$1
          }), endVoid = getVoid(editor, {
            at: end$1
          });
      ...
      onCompositionEnd: useCallback((event) => {
        if (!isDOMEventTargetInput(event) && ReactEditor.hasSelectableTarget(editor, event.target)) {
          if (ReactEditor.isComposing(editor) && Promise.resolve().then(() => {
            setIsComposing(!1), editor.composing = !1;
          ...
          if (!IS_WEBKIT && !IS_FIREFOX_LEGACY && !IS_IOS && !IS_WECHATBROWSER && !IS_UC_MOBILE && event.data) {
            ...
            editor.pendingInsertionMarks = null, placeholderMarks !== void 0 && (editor.userMarks = editor.marks, editor.marks = placeholderMarks), editorActor.send({
              type: "behavior event",
              behaviorEvent: {
                type: "insert.text",
                text: event.data
            ...
            const userMarks = editor.userMarks;
            editor.userMarks = null, userMarks !== void 0 && (editor.marks = userMarks);
          }
        }
      }, [attributes.onCompositionEnd, editor, editorActor]),
      onCompositionUpdate: useCallback((event) => {
        ReactEditor.hasSelectableTarget(editor, event.target) && !isEventHandled(event, attributes.onCompositionUpdate) && !isDOMEventTargetInput(event) && (ReactEditor.isComposing(editor) || (setIsComposing(!0), editor.composing = !0));
      ...
      onKeyDown: useCallback((event) => {
        if (!readOnly && ReactEditor.hasEditableTarget(editor, event.target)) {
          ...
          if (ReactEditor.isComposing(editor) && nativeEvent.isComposing === !1 && (editor.composing = !1, setIsComposing(!1)), isEventHandled(event, attributes.onKeyDown) || ReactEditor.isComposing(editor))
            return;
          const {
            selection
          } = editor, element = editor.children[selection !== null ? selection.focus.path[0] : 0], isRTL = getDirection(getString(element, editor.schema)) === "rtl";
          ...
          if (Hotkeys.isMoveBackward(nativeEvent)) {
            event.preventDefault(), selection && Range.isCollapsed(selection) ? move(editor, {
              reverse: !isRTL
            }) : collapse(editor, {
          ...
          if (Hotkeys.isMoveWordBackward(nativeEvent)) {
            event.preventDefault(), selection && Range.isExpanded(selection) && collapse(editor, {
              edge: "focus"
            }), move(editor, {
              unit: "word",
              reverse: !isRTL
          ...
          if (HAS_BEFORE_INPUT_SUPPORT) {
            ...
            if (Hotkeys.isDeleteBackward(nativeEvent)) {
              event.preventDefault(), selection && Range.isExpanded(selection) ? editorActor.send({
                type: "behavior event",
                behaviorEvent: {
                  type: "delete",
                  direction: "backward"
      ...
      onPaste: useCallback((event) => {
        !readOnly && ReactEditor.hasEditableTarget(editor, event.target) && !isEventHandled(event, attributes.onPaste) && (!HAS_BEFORE_INPUT_SUPPORT || isPlainTextOnlyPaste(event.nativeEvent) || IS_WEBKIT) && (event.preventDefault(), editorActor.send({
          type: "behavior event",
          behaviorEvent: {
            type: "input.*",
            originEvent: {
...
function createDecorate(schema, slateEditor) {
  return function([node2, path2]) {
    ...
    if (slateEditor.children.length === 1 && firstBlock && isEmptyTextBlock({
      schema
    ...
    if (path2.length === 0)
      return [];
    if (!Element$2.isElement(node2, slateEditor.schema) || node2.children.length === 0)
      return [];
...
function SelectionStateProvider(t0) {
  ...
  const selectionState = useSelector(editorActor, t1, _temp2$2);
  let t2;
...
function RenderBlockObject(props) {
  ...
  let t1;
  if ($[3] !== props.element._type || $[4] !== props.schema.blockObjects) {
    let t22;
    $[6] !== props.element._type ? (t22 = (schemaType) => schemaType.name === props.element._type, $[6] = props.element._type, $[7] = t22) : t22 = $[7], t1 = props.schema.blockObjects.find(t22), $[3] = props.element._type, $[4] = props.schema.blockObjects, $[5] = t1;
  } else
    t1 = $[5];
  ...
  let t5;
  $[14] !== blockObject || $[15] !== blockObjectSchemaType || $[16] !== focused || $[17] !== props.element._key || $[18] !== props.renderBlock || $[19] !== selected ? (t5 = props.renderBlock && blockObjectSchemaType ? /* @__PURE__ */ jsx(RenderBlock$1, { renderBlock: props.renderBlock, editorElementRef: blockObjectRef, focused, path: [{
    _key: props.element._key
  }], schemaType: blockObjectSchemaType, selected, value: blockObject, children: /* @__PURE__ */ jsx(RenderDefaultBlockObject, { blockObject }) }) : /* @__PURE__ */ jsx(RenderDefaultBlockObject, { blockObject }), $[14] = blockObject, $[15] = blockObjectSchemaType, $[16] = focused, $[17] = props.element._key, $[18] = props.renderBlock, $[19] = selected, $[20] = t5) : t5 = $[20];
  let t6;
...
function RenderInlineObject(props) {
  const $ = c(22), inlineObjectRef = useRef(null), slateEditor = useSlateStatic();
  let t0;
  if ($[0] !== props.element || $[1] !== props.schema.inlineObjects) {
    let t12;
    $[3] !== props.element ? (t12 = (schemaType) => schemaType.name === props.element._type, $[3] = props.element, $[4] = t12) : t12 = $[4], t0 = props.schema.inlineObjects.find(t12), $[0] = props.element, $[1] = props.schema.inlineObjects, $[2] = t0;
  } else
    t0 = $[2];
  ...
  const path2 = block ? [{
    ...
    _key: props.element._key
  }] : void 0, selectionState = useContext(SelectionStateContext), serializedPath = path2 ? serializePath(path2) : void 0, selected = serializedPath ? selectionState.selectedChildPaths.has(serializedPath) : !1, focused = serializedPath ? selectionState.focusedChildPath === serializedPath : !1, inlineObject = props.element;
  let t1;
  $[5] === /* @__PURE__ */ Symbol.for("react.memo_cache_sentinel") ? (t1 = {
    display: "inline-block"
  }, $[5] = t1) : t1 = $[5];
  const t2 = !props.readOnly;
  let t3;
  $[6] !== focused || $[7] !== inlineObject || $[8] !== inlineObjectSchemaType || $[9] !== path2 || $[10] !== props.renderChild || $[11] !== selected ? (t3 = props.renderChild && path2 && inlineObjectSchemaType ? /* @__PURE__ */ jsx(RenderChild$1, { renderChild: props.renderChild, annotations: [], editorElementRef: inlineObjectRef, selected, focused, path: path2, schemaType: inlineObjectSchemaType, value: inlineObject, children: /* @__PURE__ */ jsx(RenderDefaultInlineObject, { inlineObject }) }) : /* @__PURE__ */ jsx(RenderDefaultInlineObject, { inlineObject }), $[6] = focused, $[7] = inlineObject, $[8] = inlineObjectSchemaType, $[9] = path2, $[10] = props.renderChild, $[11] = selected, $[12] = t3) : t3 = $[12];
  let t4;
  $[13] !== t2 || $[14] !== t3 ? (t4 = /* @__PURE__ */ jsx("span", { ref: inlineObjectRef, style: t1, draggable: t2, children: t3 }), $[13] = t2, $[14] = t3, $[15] = t4) : t4 = $[15];
  let t5;
  return $[16] !== inlineObject._key || $[17] !== inlineObject._type || $[18] !== props.attributes || $[19] !== props.children || $[20] !== t4 ? (t5 = /* @__PURE__ */ jsxs("span", { ...props.attributes, className: "pt-inline-object", "data-child-key": inlineObject._key, "data-child-name": inlineObject._type, "data-child-type": "object", children: [
    props.children,
    t4
  ] }), $[16] = inlineObject._key, $[17] = inlineObject._type, $[18] = props.attributes, $[19] = props.children, $[20] = t4, $[21] = t5) : t5 = $[21], t5;
}
...
function RenderTextBlock(props) {
  const $ = c(81), blockRef = useRef(null);
  let t0;
  ...
  let t2;
  $[5] !== props.textBlock._key || $[6] !== selectedBlockKeys ? (t2 = selectedBlockKeys.has(props.textBlock._key), $[5] = props.textBlock._key, $[6] = selectedBlockKeys, $[7] = t2) : t2 = $[7];
  const selected = t2, focused = focusedBlockKey === props.textBlock._key;
  let t3;
  $[8] !== props.textBlock._key ? (t3 = (editor) => editor.listIndexMap.get(props.textBlock._key), $[8] = props.textBlock._key, $[9] = t3) : t3 = $[9];
  const listIndex = useSlateSelector(t3);
  let children = props.children;
  if (props.renderStyle && props.textBlock.style) {
    let t42;
    $[10] !== props.schema.styles || $[11] !== props.textBlock.style ? (t42 = props.textBlock.style !== void 0 ? props.schema.styles.find((style) => style.value === props.textBlock.style) : void 0, $[10] = props.schema.styles, $[11] = props.textBlock.style, $[12] = t42) : t42 = $[12];
    const styleSchemaType = t42;
    if (styleSchemaType) {
      let t52;
      $[13] !== props.textBlock._key ? (t52 = [{
        _key: props.textBlock._key
      }], $[13] = props.textBlock._key, $[14] = t52) : t52 = $[14];
      let t62;
      $[15] !== children || $[16] !== focused || $[17] !== props.renderStyle || $[18] !== props.textBlock || $[19] !== selected || $[20] !== styleSchemaType || $[21] !== t52 ? (t62 = /* @__PURE__ */ jsx(RenderStyle, { renderStyle: props.renderStyle, block: props.textBlock, editorElementRef: blockRef, focused, path: t52, schemaType: styleSchemaType, selected, value: props.textBlock.style, children }), $[15] = children, $[16] = focused, $[17] = props.renderStyle, $[18] = props.textBlock, $[19] = selected, $[20] = styleSchemaType, $[21] = t52, $[22] = t62) : t62 = $[22], children = t62;
    } else
      console.error(`Unable to find Schema type for text block style ${props.textBlock.style}`);
  }
  if (props.renderListItem && props.textBlock.listItem) {
    let t42;
    if ($[23] !== props.schema.lists || $[24] !== props.textBlock.listItem) {
      let t52;
      $[26] !== props.textBlock.listItem ? (t52 = (list) => list.value === props.textBlock.listItem, $[26] = props.textBlock.listItem, $[27] = t52) : t52 = $[27], t42 = props.schema.lists.find(t52), $[23] = props.schema.lists, $[24] = props.textBlock.listItem, $[25] = t42;
    } else
      t42 = $[25];
    ...
    if (listItemSchemaType) {
      ...
      let t62;
      $[28] !== props.textBlock._key ? (t62 = [{
        _key: props.textBlock._key
      }], $[28] = props.textBlock._key, $[29] = t62) : t62 = $[29];
      let t72;
      $[30] !== children || $[31] !== focused || $[32] !== listItemSchemaType || $[33] !== props.renderListItem || $[34] !== props.textBlock || $[35] !== selected || $[36] !== t52 || $[37] !== t62 ? (t72 = /* @__PURE__ */ jsx(RenderListItem, { renderListItem: props.renderListItem, block: props.textBlock, editorElementRef: blockRef, focused, level: t52, path: t62, selected, value: props.textBlock.listItem, schemaType: listItemSchemaType, children }), $[30] = children, $[31] = focused, $[32] = listItemSchemaType, $[33] = props.renderListItem, $[34] = props.textBlock, $[35] = selected, $[36] = t52, $[37] = t62, $[38] = t72) : t72 = $[38], children = t72;
    } else
      console.error(`Unable to find Schema type for text block list item ${props.textBlock.listItem}`);
  }
  const t4 = props.attributes;
  let t5;
  $[39] !== props.textBlock.style ? (t5 = props.textBlock.style ? [`pt-text-block-style-${props.textBlock.style}`] : [], $[39] = props.textBlock.style, $[40] = t5) : t5 = $[40];
  let t6;
  $[41] !== props.textBlock.level || $[42] !== props.textBlock.listItem ? (t6 = props.textBlock.listItem ? ["pt-list-item", `pt-list-item-${props.textBlock.listItem}`, `pt-list-item-level-${props.textBlock.level ?? 1}`] : [], $[41] = props.textBlock.level, $[42] = props.textBlock.listItem, $[43] = t6) : t6 = $[43];
  let t7;
  $[44] !== t5 || $[45] !== t6 ? (t7 = ["pt-block", "pt-text-block", ...t5, ...t6], $[44] = t5, $[45] = t6, $[46] = t7) : t7 = $[46];
  const t8 = t7.join(" ");
  let t9;
  $[47] !== props.textBlock.listItem ? (t9 = props.textBlock.listItem !== void 0 ? {
    "data-list-item": props.textBlock.listItem
  } : {}, $[47] = props.textBlock.listItem, $[48] = t9) : t9 = $[48];
  let t10;
  $[49] !== props.textBlock.level ? (t10 = props.textBlock.level !== void 0 ? {
    "data-level": props.textBlock.level
  } : {}, $[49] = props.textBlock.level, $[50] = t10) : t10 = $[50];
  let t11;
  $[51] !== props.textBlock.style ? (t11 = props.textBlock.style !== void 0 ? {
    "data-style": props.textBlock.style
  } : {}, $[51] = props.textBlock.style, $[52] = t11) : t11 = $[52];
  let t12;
  $[53] !== listIndex ? (t12 = listIndex !== void 0 ? {
    "data-list-index": listIndex
  } : {}, $[53] = listIndex, $[54] = t12) : t12 = $[54];
  let t13;
  $[55] !== props.dropPosition ? (t13 = props.dropPosition === "start" ? /* @__PURE__ */ jsx(DropIndicator, {}) : null, $[55] = props.dropPosition, $[56] = t13) : t13 = $[56];
  let t14;
  $[57] !== children || $[58] !== focused || $[59] !== props.renderBlock || $[60] !== props.textBlock || $[61] !== schemaType || $[62] !== selected ? (t14 = props.renderBlock ? /* @__PURE__ */ jsx(RenderBlock, { renderBlock: props.renderBlock, editorElementRef: blockRef, focused, level: props.textBlock.level, listItem: props.textBlock.listItem, path: [{
    _key: props.textBlock._key
  }], selected, schemaType, style: props.textBlock.style, value: props.textBlock, children }) : children, $[57] = children, $[58] = focused, $[59] = props.renderBlock, $[60] = props.textBlock, $[61] = schemaType, $[62] = selected, $[63] = t14) : t14 = $[63];
  let t15;
  $[64] !== t14 ? (t15 = /* @__PURE__ */ jsx("div", { ref: blockRef, children: t14 }), $[64] = t14, $[65] = t15) : t15 = $[65];
  let t16;
  $[66] !== props.dropPosition ? (t16 = props.dropPosition === "end" ? /* @__PURE__ */ jsx(DropIndicator, {}) : null, $[66] = props.dropPosition, $[67] = t16) : t16 = $[67];
  let t17;
  return $[68] !== props.attributes || $[69] !== props.spellCheck || $[70] !== props.textBlock._key || $[71] !== props.textBlock._type || $[72] !== t10 || $[73] !== t11 || $[74] !== t12 || $[75] !== t13 || $[76] !== t15 || $[77] !== t16 || $[78] !== t8 || $[79] !== t9 ? (t17 = /* @__PURE__ */ jsxs("div", { ...t4, className: t8, spellCheck: props.spellCheck, "data-block-key": props.textBlock._key, "data-block-name": props.textBlock._type, "data-block-type": "text", ...t9, ...t10, ...t11, ...t12, children: [
    t13,
    t15,
    t16
  ] }), $[68] = props.attributes, $[69] = props.spellCheck, $[70] = props.textBlock._key, $[71] = props.textBlock._type, $[72] = t10, $[73] = t11, $[74] = t12, $[75] = t13, $[76] = t15, $[77] = t16, $[78] = t8, $[79] = t9, $[80] = t17) : t17 = $[80], t17;
}
...
function RenderElement(props) {
  const $ = c(34), editorActor = useContext(EditorActorContext), schema = useSelector(editorActor, _temp$3), slateStatic = useSlateStatic();
  if (slateStatic.isInline(props.element)) {
    let t02;
    return $[0] !== props.attributes || $[1] !== props.children || $[2] !== props.element || $[3] !== props.readOnly || $[4] !== props.renderChild || $[5] !== schema ? (t02 = /* @__PURE__ */ jsx(RenderInlineObject, { attributes: props.attributes, element: props.element, readOnly: props.readOnly, renderChild: props.renderChild, schema, children: props.children }), $[0] = props.attributes, $[1] = props.children, $[2] = props.element, $[3] = props.readOnly, $[4] = props.renderChild, $[5] = schema, $[6] = t02) : t02 = $[6], t02;
  }
  let block, t0;
  ...
  let t2;
  return $[25] !== block || $[26] !== props.attributes || $[27] !== props.children || $[28] !== props.element || $[29] !== props.readOnly || $[30] !== props.renderBlock || $[31] !== schema || $[32] !== t1 ? (t2 = /* @__PURE__ */ jsx(RenderBlockObject, { attributes: props.attributes, blockObject: block, dropPosition: t1, element: props.element, readOnly: props.readOnly, renderBlock: props.renderBlock, schema, children: props.children }), $[25] = block, $[26] = props.attributes, $[27] = props.children, $[28] = props.element, $[29] = props.readOnly, $[30] = props.renderBlock, $[31] = schema, $[32] = t1, $[33] = t2) : t2 = $[33], t2;
}
function _temp$3(s) {
  return s.context.schema;
}
function RenderSpan(props) {
  const $ = c(40), slateEditor = useSlateStatic(), editorActor = useContext(EditorActorContext), schema = useSelector(editorActor, _temp$2), spanRef = useRef(null);
  let t0;
  ...
  const schemaType = t1, parent2 = props.children.props.parent, block = parent2 && slateEditor.isTextBlock(parent2) ? parent2 : void 0;
  let t2;
  $[3] !== block || $[4] !== props.leaf._key ? (t2 = block ? [{
    _key: block._key
  }, "children", {
    _key: props.leaf._key
  }] : void 0, $[3] = block, $[4] = props.leaf._key, $[5] = t2) : t2 = $[5];
  const path2 = t2, selectionState = useContext(SelectionStateContext);
  let focused, t3;
  if ($[6] !== path2 || $[7] !== selectionState) {
    const serializedPath = path2 ? serializePath(path2) : void 0;
    focused = serializedPath ? selectionState.focusedChildPath === serializedPath : !1, t3 = serializedPath ? selectionState.selectedChildPaths.has(serializedPath) : !1, $[6] = path2, $[7] = selectionState, $[8] = focused, $[9] = t3;
  } else
    focused = $[8], t3 = $[9];
  const selected = t3;
  let annotationMarkDefs, children;
  if ($[10] !== block || $[11] !== editorActor || $[12] !== focused || $[13] !== path2 || $[14] !== props.children || $[15] !== props.leaf.marks || $[16] !== props.renderAnnotation || $[17] !== props.renderDecorator || $[18] !== schema.annotations || $[19] !== schema.decorators || $[20] !== selected) {
    ...
    for (const mark_1 of decorators) {
      const decoratorSchemaType = schema.decorators.find((dec) => dec.name === mark_1);
      path2 && decoratorSchemaType && props.renderDecorator && (children = /* @__PURE__ */ jsx(RenderDecorator, { renderDecorator: props.renderDecorator, editorElementRef: spanRef, focused, path: path2, selected, schemaType: decoratorSchemaType, value: mark_1, children }));
    }
    for (const annotationMarkDef of annotationMarkDefs) {
      const annotationSchemaType = schema.annotations.find((t) => t.name === annotationMarkDef._type);
      annotationSchemaType && (block && path2 && props.renderAnnotation ? children = /* @__PURE__ */ jsx("span", { ref: spanRef, children: /* @__PURE__ */ jsx(RenderAnnotation, { renderAnnotation: props.renderAnnotation, block, editorElementRef: spanRef, focused, path: path2, selected, schemaType: annotationSchemaType, value: annotationMarkDef, children }) }) : children = /* @__PURE__ */ jsx("span", { ref: spanRef, children }));
    }
    $[10] = block, $[11] = editorActor, $[12] = focused, $[13] = path2, $[14] = props.children, $[15] = props.leaf.marks, $[16] = props.renderAnnotation, $[17] = props.renderDecorator, $[18] = schema.annotations, $[19] = schema.decorators, $[20] = selected, $[21] = annotationMarkDefs, $[22] = children;
  } else
    annotationMarkDefs = $[21], children = $[22];
  if (block && path2 && props.renderChild) {
    let t42;
    if ($[23] !== block.children || $[24] !== props.leaf) {
      let t5;
      $[26] !== props.leaf ? (t5 = (_child) => _child._key === props.leaf._key, $[26] = props.leaf, $[27] = t5) : t5 = $[27], t42 = block.children.find(t5), $[23] = block.children, $[24] = props.leaf, $[25] = t42;
    } else
      t42 = $[25];
    const child = t42;
    if (child) {
      let t5;
      $[28] !== annotationMarkDefs || $[29] !== child || $[30] !== children || $[31] !== focused || $[32] !== path2 || $[33] !== props.renderChild || $[34] !== schemaType || $[35] !== selected ? (t5 = /* @__PURE__ */ jsx(RenderChild, { renderChild: props.renderChild, annotations: annotationMarkDefs, editorElementRef: spanRef, focused, path: path2, schemaType, selected, value: child, children }), $[28] = annotationMarkDefs, $[29] = child, $[30] = children, $[31] = focused, $[32] = path2, $[33] = props.renderChild, $[34] = schemaType, $[35] = selected, $[36] = t5) : t5 = $[36], children = t5;
    }
  }
  let t4;
  return $[37] !== children || $[38] !== props.attributes ? (t4 = /* @__PURE__ */ jsx("span", { ...props.attributes, ref: spanRef, children }), $[37] = children, $[38] = props.attributes, $[39] = t4) : t4 = $[39], t4;
}
...
function applySelect(editor, target) {
  ...
  if (selection) {
    ...
    range2.anchor != null && !Point.equals(range2.anchor, selection.anchor) && (oldProps.anchor = selection.anchor, newProps.anchor = range2.anchor), range2.focus != null && !Point.equals(range2.focus, selection.focus) && (oldProps.focus = selection.focus, newProps.focus = range2.focus), Object.keys(oldProps).length > 0 && editor.apply({
      type: "set_selection",
      properties: oldProps,
      newProperties: newProps
    });
...
function validateSelection(slateEditor, editorElement) {
  ...
  try {
    ...
    debug$1.selection("Could not resolve selection, selecting top document"), applyDeselect(slateEditor), slateEditor.children.length > 0 && applySelect(slateEditor, start(slateEditor, [])), slateEditor.onChange();
  }
}
const PortableTextEditable = forwardRef(function(props, forwardedRef) {
  ...
  $[64] !== editorActor || $[65] !== propsSelection || $[66] !== slateEditor ? (t13 = () => {
    if (propsSelection) {
      ...
      if (normalizedSelection !== null) {
        ...
        slateRange && (slateEditor.select(slateRange), slateEditor.operations.some(_temp4) || editorActor.send({
  ...
  $[86] !== editorActor || $[87] !== onPaste || $[88] !== portableTextEditor || $[89] !== relayActor || $[90] !== slateEditor ? (t20 = (event_1) => {
    const value = slateEditor.children, path2 = (slateEditor.selection ? slateRangeToSelection({
      schema: editorActor.getSnapshot().context.schema,
      editor: slateEditor,
      range: slateEditor.selection
...
function createEditableAPI(editor, editorActor) {
  return {
    ...
    focusChild: () => {
      if (!editor.selection)
        return;
      const focusBlockIndex = editor.selection.focus.path.at(0), focusChildIndex = editor.selection.focus.path.at(1), block = focusBlockIndex !== void 0 ? editor.children.at(focusBlockIndex) : void 0;
    ...
    findDOMNode: (element) => {
      let node2;
      try {
        const [item] = Array.from(nodes(editor, {
          at: [],
    ...
    activeAnnotations: () => {
      ...
      try {
        const activeAnnotations = [], spans = nodes(editor, {
          at: editor.selection,
          match: (node2) => Text$1.isText(node2, editor.schema) && node2.marks !== void 0 && Array.isArray(node2.marks) && node2.marks.length > 0
        });
        for (const [span, path2] of spans) {
          ...
          editor.isTextBlock(block) && block.markDefs?.forEach((def) => {
            Text$1.isText(span, editor.schema) && span.marks && Array.isArray(span.marks) && span.marks.includes(def._key) && activeAnnotations.push(def);
          });
        }
...
function applyInsertNodeAtPoint(editor, node2, at) {
  withoutNormalizing(editor, () => {
    const match2 = Text$1.isText(node2, editor.schema) ? (n2) => Text$1.isText(n2, editor.schema) : (n2) => Text$1.isText(n2, editor.schema) || editor.isObjectNode(n2), [entry] = nodes(editor, {
      at: at.path,
      match: match2,
      mode: "lowest",
      voids: !1
    ...
    if (!entry)
      return;
    const [, matchPath2] = entry, ref = pathRef(editor, matchPath2), isAtEnd = isEnd(editor, at, matchPath2);
    if (!isEdge(editor, at, matchPath2)) {
      const textNode = getNode(editor, at.path, editor.schema), properties = extractProps(textNode, editor.schema);
      applySplitNode(editor, at.path, at.offset, properties);
    }
    const path2 = ref.unref(), insertPath = isAtEnd ? Path.next(path2) : path2;
    editor.apply({
      type: "insert_node",
      path: insertPath,
      node: node2
...
function* getChildren(root, path2, schema, options = {}) {
  const {
    reverse = !1
  } = options, ancestor = getAncestor(root, path2, schema), {
    children
  } = ancestor;
...
function createNormalizationPlugin(editorActor) {
  return function(editor) {
    ...
    return editor.normalizeNode = (nodeEntry) => {
      const [node$1, path2] = nodeEntry;
      if (isEditor(node$1) && node$1.children.length === 0 && withoutPatching(editor, () => {
        withNormalizeNode(editor, () => {
          applyInsertNodeAtPath(editor, createPlaceholderBlock(editorActor.getSnapshot().context), [0]);
        });
      }), editor.isTextBlock(node$1)) {
      ...
      if (editor.isTextBlock(node$1) && !Array.isArray(node$1.markDefs)) {
      ...
      if (defaultStyle && editor.isTextBlock(node$1) && typeof node$1.style > "u") {
      ...
      if (editor.isTextSpan(node$1) && !Array.isArray(node$1.marks)) {
      ...
      if (editor.isTextSpan(node$1)) {
        const blockPath = Path.parent(path2), [block] = node(editor, blockPath), decorators = editorActor.getSnapshot().context.schema.decorators.map((decorator) => decorator.name), annotations = node$1.marks?.filter((mark) => !decorators.includes(mark));
        if (editor.isTextBlock(block) && node$1.text === "" && annotations && annotations.length > 0) {
      ...
      if (editor.isTextSpan(node$1)) {
        const blockPath = Path.parent(path2), [block] = node(editor, blockPath);
      ...
      if (editor.isTextBlock(node$1)) {
        const newMarkDefs = (node$1.markDefs || []).filter((def) => node$1.children.find((child) => Text$1.isText(child, editor.schema) && Array.isArray(child.marks) && child.marks.includes(def._key)));
...
function toSlateBlock(block, {
  ...
  if (block && block._type === schemaTypes.block.name) {
    ...
    return !hasMissingMarkDefs && !hasMissingChildren && !hasInlines2 && Element$2.isElement(block, schemaTypes) ? block : {
      _type,
      _key,
      ...rest,
      children
...
function isEqualToEmptyEditor(initialValue, blocks, schemaTypes) {
  ...
  if (!firstBlock)
    return !0;
  if (!Element$2.isElement(firstBlock, schemaTypes) || firstBlock._type !== schemaTypes.block.name || "listItem" in firstBlock || !("style" in firstBlock) || firstBlock.style !== schemaTypes.styles.at(0)?.name || !Array.isArray(firstBlock.children) || firstBlock.children.length !== 1)
    return !1;
  const firstChild = firstBlock.children.at(0);
  return !(!firstChild || !Text$1.isText(firstChild, schemaTypes) || !("_type" in firstChild) || firstChild._type !== schemaTypes.span.name || firstChild.text !== "" || firstChild.marks?.join("") || "markDefs" in firstBlock && Array.isArray(firstBlock.markDefs) && firstBlock.markDefs.length > 0 || Object.keys(firstBlock).some((key) => key !== "_type" && key !== "_key" && key !== "children" && key !== "markDefs" && key !== "style") || isEqualValues({
    schema: schemaTypes
...
function diffMatchPatch(editor, patch) {
  ...
  if (!child || !(block && editor.isTextBlock(block.node) && patch.path.length === 4 && patch.path[1] === "children" && patch.path[3] === "text") || !Text$1.isText(child.node, editor.schema))
    return !1;
...
function insertPatch(context, editor, patch) {
  const block = findBlock(editor.children, patch.path);
  if (!block)
    return patch.path.length === 1 && patch.path[0] === 0 ? (patch.items.map((item) => toSlateBlock(item, {
  ...
  if (patch.path.length === 1) {
    const {
      ...
      schemaTypes: context.schema
    })), targetBlockIndex = block.index, normalizedIdx2 = position2 === "after" ? targetBlockIndex + 1 : targetBlockIndex, editorWasEmptyBefore = isEqualToEmptyEditor(context.initialValue, editor.children, context.schema);
    if (blocksToInsert.forEach((node2, i) => {
      editor.apply({
        type: "insert_node",
        path: [normalizedIdx2 + i],
        node: node2
      });
    }), editorWasEmptyBefore && typeof patch.path[0] == "number" && patch.path[0] === 0) {
      const removeIdx = position2 === "before" ? targetBlockIndex + blocksToInsert.length : targetBlockIndex, [removeNode] = node(editor, [removeIdx]);
      editor.apply({
        type: "remove_node",
        path: [removeIdx],
        node: removeNode
  ...
  const {
    items,
    position
  } = patch, targetChild = findBlockChild(block, patch.path, editor.schema);
  if (!targetChild)
    return !1;
  ...
  return childrenToInsert && Element$2.isElement(childrenToInsert, editor.schema) && childrenToInsert.children.forEach((node2, i) => {
    editor.apply({
      type: "insert_node",
...
function setPatch(editor, patch) {
  ...
  if (patch.path.length === 1) {
    ...
    if (editor.isTextBlock(block.node) && Element$2.isElement(updatedBlock, editor.schema)) {
      ...
      for (const pathRef2 of childPaths) {
        const childPath = pathRef2.unref();
        if (childPath) {
          const [childNode] = node(editor, childPath);
          editor.apply({
            type: "remove_node",
            path: childPath,
            node: childNode
      ...
      return updatedBlock.children.forEach((node2, i) => {
        editor.apply({
          type: "insert_node",
          path: [block.index, i],
          node: node2
        });
      }), previousSelection && applySelect(editor, previousSelection), !0;
    } else
  ...
  if (isTextBlock2 && child) {
    if (Text$1.isText(child.node, editor.schema))
      ...
      const propPath = patch.path.slice(3), reservedProps = ["_key", "_type", "children"], propEntry = propPath.at(0);
...
function unsetPatch(editor, patch) {
  if (patch.path.length === 0) {
    ...
    for (const [node2, path2] of children)
      editor.apply({
        type: "remove_node",
        path: path2,
        node: node2
  ...
  if (!block)
    return !1;
  if (patch.path.length === 1) {
    const [blockNode] = node(editor, [block.index]);
    return editor.apply({
      type: "remove_node",
      path: [block.index],
      node: blockNode
  ...
  if (child && (Element$2.isElement(child.node, editor.schema) || editor.isObjectNode(child.node))) {
  ...
  if (child && Text$1.isText(child.node, editor.schema)) {
    const propPath = patch.path.slice(3), propEntry = propPath.at(0);
    ...
    const newNode = applyAll(child.node, [{
      ...patch,
      path: propPath
    }]), newKeys = Object.keys(newNode), removedProperties = Object.keys(child.node).filter((property) => !newKeys.includes(property)), unsetProps = {};
    for (const prop of removedProperties)
      unsetProps[prop] = null;
  ...
  if (!child) {
    if (!editor.isTextBlock(block.node)) {
      const newVal = applyAll(block.node, [{
        ...patch,
        path: patch.path.slice(1)
      }]), newKeys = Object.keys(newVal), removedProperties = Object.keys(block.node).filter((property) => !newKeys.includes(property));
      if (removedProperties.length > 0) {
        const unsetProps = {};
        for (const prop of removedProperties)
          unsetProps[prop] = null;
...
function findBlockChild(block, path2, schema) {
  ...
  if (!Element$2.isElement(blockNode, schema) || path2[1] !== "children")
    return;
  let childIndex = -1;
  ...
  if (blockIndex !== void 0 && operation.path.length === 1) {
    ...
    if (isTextBlock({
      ...
      for (const [key, propertyValue] of Object.entries(operation.newProperties))
        key === "_key" ? patches.push(set(propertyValue, [blockIndex, "_key"])) : patches.push(set(propertyValue, [{
          _key: block._key
        }, key]));
    ...
    if (isTextBlock({
      ...
      if (child) {
        ...
        if (Element$2.isElement(child, schema)) {
        ...
        for (const [keyName, propertyValue] of Object.entries(operation.newProperties)) {
          if (keyName === "_key") {
            patches.push(set(propertyValue, [{
              _key: blockKey
...
function insertNodePatch(schema, children, operation, beforeValue) {
  ...
  if (operation.path.length === 1) {
    ...
    return targetKey ? [insert([operation.node], position, [{
      _key: targetKey
    }])] : [setIfMissing(beforeValue, []), insert([operation.node], "before", [operation.path[0]])];
  } else if (isTextBlock({
    schema
    ...
    return Text$1.isText(operation.node, schema) ? [setIfMissingPatch, insert([operation.node], position, path2)] : [setIfMissingPatch, insert([operation.node], position, path2)];
  }
  return [];
}
...
function createPatchesPlugin({
  ...
  return function(editor) {
    ...
    return subscriptions.push(() => {
      ...
      apply2(operation);
      const editorIsEmpty = isEqualToEmptyEditor(editorActor.getSnapshot().context.initialValue, editor.children, editorActor.getSnapshot().context.schema);
...
function createSchemaPlugin({
  ...
  return function(editor) {
    editor.isTextBlock = (value) => isEditor(value) ? !1 : isTextBlock(editorActor.getSnapshot().context, value), editor.isTextSpan = (value) => isEditor(value) ? !1 : isSpan(editorActor.getSnapshot().context, value), editor.isListBlock = (value) => isEditor(value) ? !1 : isListBlock(editorActor.getSnapshot().context, value), editor.schema = editorActor.getSnapshot().context.schema, editor.isObjectNode = (value) => {
...
function createUniqueKeysPlugin(editorActor) {
  ...
  return function(editor) {
    ...
    return editor.apply = (operation) => {
      ...
      if (Element$2.isElement(node2, editor.schema) && node2._type === editorActor.getSnapshot().context.schema.block.name) {
        if (!node2._key) {
          withNormalizeNode(editor, () => {
            applySetNode(editor, {
...
${safeStringify(operation, 2)}`), operation.type === "set_selection") {
    ...
    apply2(operation), buildIndexMaps({
      schema: context.schema,
      value: editor.children
    }, {
      blockIndexMap: editor.blockIndexMap,
...
function getBlockNodes(slateEditor, snapshot) {
  ...
  if (!range2)
    return [];
  try {
    return Array.from(nodes(slateEditor, {
      at: range2,
      mode: "highest",
      match: (n2) => !isEditor(n2)
...
const addAnnotationOperationImplementation = ({
  ...
  for (const [block, blockPath] of selectedBlocks) {
    ...
    markDefs.find((markDef) => markDef._type === parsedAnnotation._type && markDef._key === annotationKey) === void 0 && applySetNode(editor, {
      markDefs: [...markDefs, {
        ...parsedAnnotation,
        _key: annotationKey
      }]
    ...
    if (splitRange && Range.isRange(splitRange)) {
      ...
      if (!(Range.isCollapsed(splitRange) && Text$1.isText(splitLeaf, editor.schema) && splitLeaf.text.length > 0)) {
        const splitRangeRef = rangeRef(editor, splitRange, {
          affinity: "inward"
        }), [splitStart, splitEnd] = Range.edges(splitRange);
        if (!isEnd(editor, splitEnd, splitEnd.path) || !isEdge(editor, splitEnd, splitEnd.path)) {
          const [endNode] = node(editor, splitEnd.path);
          applySplitNode(editor, splitEnd.path, splitEnd.offset, extractProps(endNode, editor.schema));
        }
        if (!isStart(editor, splitStart, splitStart.path) || !isEdge(editor, splitStart, splitStart.path)) {
          const [startNode] = node(editor, splitStart.path);
          applySplitNode(editor, splitStart.path, splitStart.offset, extractProps(startNode, editor.schema));
        }
  ...
  const childEntry = node(operation.editor, location, {
    depth: 2
  ...
  if (operation.editor.isTextSpan(child)) {
    ...
    applySetNode(operation.editor, {
      ...child,
      ...rest
    }, childPath), typeof text == "string" && child.text !== text && (operation.editor.apply({
      type: "remove_text",
      path: childPath,
      offset: 0,
      text: child.text
  ...
  if (childIndex === -1)
    throw new Error(`Unable to find child at ${safeStringify(operation.at)}`);
  const childEntry = node(operation.editor, [blockIndex, childIndex], {
    depth: 2
  }), child = childEntry?.[0], childPath = childEntry?.[1];
  if (!child || !childPath)
    throw new Error(`Unable to find child at ${safeStringify(operation.at)}`);
  ...
  if (at) {
    if (Range.isExpanded(at)) {
      ...
      if (!isEnd(editor, end2, end2.path) || !isEdge(editor, end2, end2.path)) {
        ...
        applySplitNode(editor, end2.path, end2.offset, extractProps(endNode, editor.schema));
      }
      if (!isStart(editor, start2, start2.path) || !isEdge(editor, start2, start2.path)) {
        const [startNode] = node(editor, start2.path);
        applySplitNode(editor, start2.path, start2.offset, extractProps(startNode, editor.schema));
      }
      if (at = ref.unref(), !at)
  ...
  if (at) {
    if (Range.isExpanded(at)) {
      ...
      if (!(Range.isCollapsed(at) && Text$1.isText(decoratorLeaf, editor.schema) && decoratorLeaf.text.length > 0)) {
        const [start2, end2] = Range.edges(at);
        if (!isEnd(editor, end2, end2.path) || !isEdge(editor, end2, end2.path)) {
          const [endNode] = node(editor, end2.path);
          applySplitNode(editor, end2.path, end2.offset, extractProps(endNode, editor.schema));
      ...
      updatedAt && [...nodes(editor, {
        ...
        Element$2.isElement(block, editor.schema) && block.children.includes(node2) && applySetNode(editor, {
  ...
  const [start$1, end$1] = Range.edges(at);
  if (operation.unit === "block") {
    const startBlockIndex = start$1.path.at(0), endBlockIndex = end$1.path.at(0);
    if (startBlockIndex === void 0 || endBlockIndex === void 0)
    ...
    const removeRange = {
      ...
      at: removeRange,
      match: (n2) => Element$2.isElement(n2, operation.editor.schema) && isBlock(operation.editor, n2) || operation.editor.isObjectNode(n2),
      mode: "highest"
    ...
    for (const pathRef2 of blockPathRefs) {
      ...
      if (path2) {
        const [node$1] = node(operation.editor, path2);
        operation.editor.apply({
          type: "remove_node",
          path: path2,
          node: node$1
  ...
  if (operation.unit === "child") {
    const childMatches = nodes(operation.editor, {
      at,
      match: (node2, path2) => isSpan(context, node2) || Element$2.isElement(node2, operation.editor.schema) && operation.editor.isInline(node2) || // TODO: Update depth check when containers land (path.length > 1 assumes flat structure)
      operation.editor.isObjectNode(node2) && path2.length > 1
  ...
  if (operation.direction === "backward" && operation.unit === "line") {
    const parentBlockEntry = above(operation.editor, {
      match: (n2) => Element$2.isElement(n2, operation.editor.schema) && isBlock(operation.editor, n2),
      at
    });
    if (parentBlockEntry) {
      const [, parentBlockPath] = parentBlockEntry, parentElementRange = range(operation.editor, parentBlockPath, at.anchor), currentLineRange = findCurrentLineRange(operation.editor, parentElementRange);
  ...
  if (Range.isCollapsed(at) && start$1.path.length >= 2)
    try {
      const node2 = getNode(operation.editor, start$1.path, operation.editor.schema);
      if (operation.editor.isObjectNode(node2)) {
        operation.editor.apply({
          type: "remove_node",
          path: start$1.path,
          node: node2
  ...
  const startNodeEntry = (() => {
    ...
    match: (n2) => Element$2.isElement(n2, operation.editor.schema) && isBlock(operation.editor, n2) || operation.editor.isObjectNode(n2),
    at: end$1
  }), isAcrossBlocks = startBlock && endBlock && !Path.equals(startBlock[1], endBlock[1]), startObjectNode = startBlock && operation.editor.isObjectNode(startBlock[0]) ? startBlock : void 0, endObjectNode = endBlock && operation.editor.isObjectNode(endBlock[0]) ? endBlock : void 0, startNonEditable = startObjectNode ?? getVoid(operation.editor, {
    at: start$1,
    mode: "highest"
  }) ?? elementReadOnly(operation.editor, {
    at: start$1,
    mode: "highest"
  }), endNonEditable = endObjectNode ?? getVoid(operation.editor, {
    at: end$1,
    mode: "highest"
  }) ?? elementReadOnly(operation.editor, {
    at: end$1,
  ...
  for (const entry of nodes(operation.editor, {
    ...
    const [node2, path2] = entry;
    lastPath && Path.compare(path2, lastPath) === 0 || (operation.editor.isObjectNode(node2) || Element$2.isElement(node2, operation.editor.schema) && Editor.isElementReadOnly(operation.editor, node2) || !Path.isCommon(path2, start$1.path) && !Path.isCommon(path2, end$1.path)) && (matches.push(entry), lastPath = path2);
  }
  ...
  if (startBlock && endBlock && Point.equals(start$1, start(operation.editor, startBlock[1])) && Point.equals(end$1, end(operation.editor, endBlock[1])) && isAcrossBlocks) {
    if (!startNonEditable) {
      const point2 = startRef.current, node2 = getNode(operation.editor, point2.path, operation.editor.schema);
      Text$1.isText(node2, operation.editor.schema) && node2.text.length > 0 && operation.editor.apply({
        type: "remove_text",
        path: point2.path,
        offset: 0,
        text: node2.text
    ...
    for (const pathRef2 of pathRefs.reverse()) {
      const path2 = pathRef2.unref();
      if (path2) {
        const [nodeAtPath] = node(operation.editor, path2);
        operation.editor.apply({
          type: "remove_node",
          path: path2,
          node: nodeAtPath
    ...
    if (!endNonEditable) {
      ...
      if (Text$1.isText(node2, operation.editor.schema)) {
        const {
          path: path2
    ...
    if (endRef.current && startRef.current) {
      ...
      for (const pathRef2 of endBlockPathRefs) {
        ...
        if (endPath) {
          const [endNode] = node(operation.editor, endPath);
          operation.editor.apply({
            type: "remove_node",
            path: endPath,
            node: endNode
    ...
    startRef.current && operation.editor.selection && operation.editor.apply({
      type: "set_selection",
  ...
  if (startNonEditable && startBlock && endBlock && Path.equals(startBlock[1], endBlock[1]) && operation.editor.isObjectNode(startBlock[0])) {
  ...
  for (const pathRef2 of [...pathRefs].reverse()) {
    ...
    if (currentPath) {
      ...
      if (operation.editor.isObjectNode(nodeAtPath)) {
        const path2 = pathRef2.unref();
        path2 && (endObjectNode && Path.equals(path2, endObjectNode[1]) && (removedEndObjectNode = !0), operation.editor.apply({
          type: "remove_node",
          path: path2,
          node: nodeAtPath
        }));
  ...
  const updatedStart = startRef.current, updatedEnd = endRef.current, updatedAt = (() => {
    if (!updatedStart)
      return at;
    if (removedEndObjectNode && (!updatedEnd || updatedStart.path.at(0) !== updatedEnd.path.at(0))) {
      const startBlockEnd = end(operation.editor, [updatedStart.path.at(0)]);
      return {
        anchor: updatedStart,
        focus: startBlockEnd
      };
  ...
  })(), reverse = operation.direction === "backward", hanging = reverse ? end$1 ? isTextBlock(context, endBlock) ? end$1.offset === 0 : !0 : !1 : start$1 ? isTextBlock(context, startBlock) ? start$1.offset === 0 : !0 : !1;
  removedEndObjectNode ? (deleteText(operation.editor, {
    at: updatedAt,
    reverse
  }), operation.editor.selection && startRef.current && operation.editor.apply({
    type: "set_selection",
    properties: operation.editor.selection,
    newProperties: {
      anchor: startRef.current,
...
function findCurrentLineRange(editor, parentRange) {
  const parentRangeBoundary = range(editor, Range.end(parentRange)), positions$1 = Array.from(positions(editor, {
    at: parentRange
  }));
...
function insertBlock(options) {
  ...
  const start$1 = at ? Range.start(at) : start(editor, []), end$1 = at ? Range.end(at) : end(editor, []), [startBlock, startBlockPath] = Array.from(nodes(editor, {
    at: start$1,
    mode: "lowest",
    match: (node2, path2) => (Element$2.isElement(node2, editor.schema) || editor.isObjectNode(node2)) && path2.length <= start$1.path.length
  })).at(0) ?? [void 0, void 0];
  let [endBlock, endBlockPath] = Array.from(nodes(editor, {
    at: end$1,
    mode: "lowest",
    match: (node2, path2) => (Element$2.isElement(node2, editor.schema) || editor.isObjectNode(node2)) && path2.length <= end$1.path.length
  })).at(0) ?? [void 0, void 0];
  ...
  if (Range.isExpanded(at) && !editor.isTextBlock(block)) {
    const atBeforeDelete = rangeRef(editor, at, {
      affinity: "inward"
    }), start2 = Range.start(at), startBlock2 = getNode(editor, [start2.path[0]], editor.schema), startOfBlock = start(editor, [start2.path[0]]), isAtStartOfBlock = Element$2.isElement(startBlock2, editor.schema) && Point.equals(start2, startOfBlock);
    ...
    if (atAfterDelete) {
      ...
      if (editor.apply({
        ...
        const point2 = start(editor, insertPath);
        editor.apply({
          type: "set_selection",
    ...
    if (select2 === "none" && atAfterInsert && editor.apply({
      ...
      try {
        ...
        Element$2.isElement(potentiallyEmptyBlock, editor.schema) && isEmptyTextBlock(context, potentiallyEmptyBlock) && editor.apply({
          type: "remove_node",
          path: emptyBlockPath,
  ...
  if (!editor.isTextBlock(block) && editor.isTextBlock(endBlock) && !Range.isExpanded(at)) {
    const selectionPoint = Range.start(at), blockPath = [selectionPoint.path[0]], blockStartPoint = start(editor, blockPath), blockEndPoint = end(editor, blockPath), isAtBlockStart = Point.equals(selectionPoint, blockStartPoint), isAtBlockEnd = Point.equals(selectionPoint, blockEndPoint);
    if (!isAtBlockStart && !isAtBlockEnd) {
      const currentBlock = getNode(editor, blockPath, editor.schema), childIndex = selectionPoint.path[1], childOffset = selectionPoint.offset;
      if (childOffset > 0) {
        const textNode = getNode(editor, selectionPoint.path, editor.schema);
      ...
      if (editor.apply({
        ...
        const firstBlockEndPoint = end(editor, blockPath);
        editor.apply({
          type: "set_selection",
          properties: editor.selection,
          newProperties: {
  ...
  if (editor.isTextBlock(endBlock) && editor.isTextBlock(block)) {
    ...
    if (Range.isExpanded(at)) {
      ...
      const [newEndBlock, newEndBlockPath] = Array.from(nodes(editor, {
        at: end(editor, []),
        mode: "lowest",
        match: (node2, path2) => (Element$2.isElement(node2, editor.schema) || editor.isObjectNode(node2)) && path2.length === 1
      })).at(-1) ?? [void 0, void 0];
      newEndBlock && newEndBlockPath && (Element$2.isElement(newEndBlock, editor.schema) || editor.isObjectNode(newEndBlock)) && (endBlock = newEndBlock, endBlockPath = newEndBlockPath);
    }
    ...
    if (Element$2.isElement(endBlockNode, editor.schema) && editor.isTextBlock(endBlockNode)) {
      const properties = {
    ...
    const adjustedBlock = isEqualChildren(block.children, adjustedChildren, context.schema.span.name) ? block : {
      ...block,
      children: adjustedChildren
    };
    if (select2 === "end") {
      const insertAt = editor.selection ? Range.end(editor.selection) : end(editor, endBlockPath);
    ...
    if (insertTextBlockFragment(editor, adjustedBlock, selectionStartPoint), select2 === "start")
      ...
      if (wasCrossBlockDeletion)
        ...
        const endBlockStartPoint = start(editor, endBlockPath);
    ...
    const endBlockStartPoint = start(editor, endBlockPath), endBlockEndPoint = end(editor, endBlockPath), selectionStartPoint = Range.start(at), selectionEndPoint = Range.end(at);
    ...
    if (Range.isExpanded(at) && Point.equals(selectionStartPoint, endBlockStartPoint)) {
      const [, end2] = Range.edges(at), endNode = getNode(editor, end2.path, editor.schema);
    ...
    if (Range.isExpanded(at) && Point.equals(selectionEndPoint, endBlockEndPoint)) {
      const [start2] = Range.edges(at), blockNode = getNode(editor, endBlockPath, editor.schema);
      for (let i = blockNode.children.length - 1; i > start2.path[1]; i--)
        removeNodeAt(editor, [...endBlockPath, i]);
      const startNode = getNode(editor, start2.path, editor.schema);
      start2.offset < startNode.text.length && editor.apply({
        type: "remove_text",
        path: start2.path,
        offset: start2.offset,
        text: startNode.text.slice(start2.offset)
      }), insertTextBlockFragment(editor, block, start2), select2 !== "none" && setSelection(editor, Path.next(endBlockPath), select2);
      return;
    }
    ...
    if (focusChild && editor.isTextSpan(focusChild)) {
      ...
      if (editor.isTextBlock(block)) {
        ...
        if (Text$1.isText(nodeToSplit, editor.schema)) {
          const {
            text: _,
            ...properties
          } = nodeToSplit;
        ...
        if (Text$1.isText(textNode, editor.schema) && currentOffset > 0 && currentOffset < textNode.text.length) {
          const {
            text: _,
            ...properties
          } = textNode;
...
function setSelection(editor, path2, position) {
  const point2 = position === "start" ? start(editor, path2) : end(editor, path2);
  editor.apply({
    type: "set_selection",
    properties: editor.selection,
    newProperties: {
...
function replaceEmptyTextBlock(editor, blockPath, newBlock, select2) {
  ...
  if (editor.apply({
    ...
    return;
  const point2 = select2 === "end" ? end(editor, blockPath) : start(editor, blockPath);
...
function deleteSameBlockRange(editor, start2, end2) {
  ...
  if (Path.equals(start2.path, end2.path)) {
    const textToRemove = getNode(editor, start2.path, editor.schema).text.slice(start2.offset, end2.offset);
    editor.apply({
      type: "remove_text",
      path: start2.path,
      offset: start2.offset,
  ...
  const newEndPath = [...blockPath, start2.path[1] + 1], endNode = getNode(editor, newEndPath, editor.schema);
  if (end2.offset > 0) {
...
function deleteCrossBlockRange(editor, start2, end2) {
  ...
  if (start2.path.length > 1) {
    ...
    const startBlock2 = getNode(editor, startBlockPath, editor.schema);
  ...
  if (end2.path.length > 1) {
    ...
    const endNodePath = [...adjustedEndBlockPath, 0], endNode = getNode(editor, endNodePath, editor.schema);
    if (end2.offset > 0) {
      const textToRemove = endNode.text.slice(0, end2.offset);
      editor.apply({
        type: "remove_text",
  ...
  editor.isTextBlock(startBlock) && editor.isTextBlock(endBlock) && withoutNormalizing(editor, () => {
...
function insertTextBlockFragment(editor, block, at) {
  if (!Element$2.isElement(block, editor.schema) || !editor.isTextBlock(block))
    return;
  if (at.offset > 0) {
    const textNode = getNode(editor, at.path, editor.schema);
    if (Text$1.isText(textNode, editor.schema)) {
      const {
        text: _,
        ...properties
      } = textNode;
...
const insertChildOperationImplementation = ({
  ...
  if (editor.isObjectNode(node2)) {
    ...
    if (hasPath(editor, nextPath)) {
      const nextNode = getNode(editor, nextPath, editor.schema);
      if (Text$1.isText(nextNode, editor.schema))
        path2 = nextPath, offset = 0, applySelect(editor, {
          path: path2,
          offset
        });
      else
...
const moveBackwardOperationImplementation = ({
  ...
  if (destinationBlockIndex === void 0)
    throw new Error("Failed to get block index from block key");
  const editor = operation.editor, node2 = getNode(editor, [originBlockIndex], editor.schema), savedSelection = editor.selection ? {
    anchor: {
  ...
  actions: {
    ...
    "handle focus": ({
      ...
      try {
        ...
        ReactEditor.focus(slateEditor), currentSelection && (slateEditor.select(currentSelection), slateEditor.pendingSelection = slateEditor.selection, slateEditor.onChange());
      } catch (error) {
      ...
      withRemoteChanges(slateEditor, () => {
        withoutPatching(slateEditor, () => {
          ...
          slateEditor.children.forEach((_, index) => {
            const removePath = [childrenLength - 1 - index], [removeNode] = node(slateEditor, removePath);
            slateEditor.apply({
              type: "remove_node",
              path: removePath,
              node: removeNode
      ...
      withoutPatching(slateEditor, () => {
        ...
        if (value.length < childrenLength) {
          for (let i = childrenLength - 1; i > value.length - 1; i--) {
            const [removeNode] = node(slateEditor, [i]);
            slateEditor.apply({
              type: "remove_node",
              path: [i],
              node: removeNode
...
function syncBlock({
  ...
  })), validation.valid || validation.resolution?.autoResolve ? (oldBlock._key === block._key && oldBlock._type === block._type ? (debug$1.syncValue("Updating block", oldBlock, block), withoutNormalizing(slateEditor, () => {
    withRemoteChanges(slateEditor, () => {
      withoutPatching(slateEditor, () => {
        updateBlock({
          context,
  ...
  })) : (debug$1.syncValue("Replacing block", oldBlock, block), withoutNormalizing(slateEditor, () => {
    withRemoteChanges(slateEditor, () => {
      withoutPatching(slateEditor, () => {
        replaceBlock({
          context,
...
function replaceBlock({
  ...
  const [oldNode] = node(slateEditor, [index]);
  slateEditor.apply({
    type: "remove_node",
    path: [index],
    node: oldNode
    ...
    path: [index],
    node: slateBlock
  }), slateEditor.onChange(), selectionFocusOnBlock && hasNode(slateEditor, currentSelection.anchor.path, slateEditor.schema) && hasNode(slateEditor, currentSelection.focus.path, slateEditor.schema) && applySelect(slateEditor, currentSelection);
}
function updateBlock({
  ...
  if (Object.keys(removedProperties).length > 0 && slateEditor.apply({
    ...
    slateBlock.children.length < oldBlockChildrenLength && Array.from(Array(oldBlockChildrenLength - slateBlock.children.length)).forEach((_, i) => {
      ...
      if (childIndex > 0) {
        debug$1.syncValue("Removing child");
        const [childNode] = node(slateEditor, [index, childIndex]);
        slateEditor.apply({
          type: "remove_node",
      ...
      const oldBlockChild = oldSlateBlock.children.at(currentBlockChildIndex), isChildChanged = !oldBlockChild || !isEqualChild(currentBlockChild, oldBlockChild, slateEditor.schema.span.name), isTextChanged = oldBlockChild && Text$1.isText(oldBlockChild, slateEditor.schema) && currentBlockChild.text !== oldBlockChild.text, path2 = [index, currentBlockChildIndex];
      if (isChildChanged)
        if (currentBlockChild._key === oldBlockChild?._key && currentBlockChild._type === oldBlockChild?._type) {
          ...
          const isSpanNode = isSpan({
            schema: context.schema
          }, currentBlockChild) && isSpan({
            schema: context.schema
          }, oldBlockChild);
          isSpanNode && isTextChanged ? (oldBlockChild.text.length > 0 && deleteText(slateEditor, {
            at: {
              focus: {
                path: path2,
                offset: 0
          ...
          }), slateEditor.onChange()) : isSpanNode || debug$1.syncValue("Updating changed inline object child", currentBlockChild);
        } else if (oldBlockChild) {
          debug$1.syncValue("Replacing child", currentBlockChild);
          const [oldChild] = node(slateEditor, [index, currentBlockChildIndex]);
          slateEditor.apply({
            type: "remove_node",
        ...
        } else oldBlockChild || (debug$1.syncValue("Inserting new child", currentBlockChild), slateEditor.apply({
          type: "insert_node",
          path: [index, currentBlockChildIndex],
          node: currentBlockChild
        }), slateEditor.onChange());
...
class PortableTextEditor {
   ...
   */
  constructor(config) {
    this.editor = config.editor, this.schemaTypes = config.editor._internal.editorActor.getSnapshot().context.schema, this.editable = config.editor._internal.editable;
  }
...
function EditorProvider(props) {
  ...
  let t2;
  $[2] !== internalEditor_0.actors.editorActor || $[3] !== internalEditor_0.actors.mutationActor || $[4] !== internalEditor_0.actors.relayActor || $[5] !== internalEditor_0.actors.syncActor || $[6] !== internalEditor_0.editor._internal.slateEditor.instance || $[7] !== internalEditor_0.subscriptions ? (t2 = () => {
    const unsubscribers = [];
  ...
  }, $[2] = internalEditor_0.actors.editorActor, $[3] = internalEditor_0.actors.mutationActor, $[4] = internalEditor_0.actors.relayActor, $[5] = internalEditor_0.actors.syncActor, $[6] = internalEditor_0.editor._internal.slateEditor.instance, $[7] = internalEditor_0.subscriptions, $[8] = t2) : t2 = $[8];
  let t3;
  ...
  let t5;
  $[14] !== internalEditor_0.editor._internal.slateEditor.initialValue || $[15] !== internalEditor_0.editor._internal.slateEditor.instance || $[16] !== t4 ? (t5 = /* @__PURE__ */ jsx(Slate, { editor: internalEditor_0.editor._internal.slateEditor.instance, initialValue: internalEditor_0.editor._internal.slateEditor.initialValue, children: t4 }), $[14] = internalEditor_0.editor._internal.slateEditor.initialValue, $[15] = internalEditor_0.editor._internal.slateEditor.instance, $[16] = t4, $[17] = t5) : t5 = $[17];
  let t6;
  $[18] !== internalEditor_0.actors.relayActor || $[19] !== t5 ? (t6 = /* @__PURE__ */ jsx(RelayActorContext.Provider, { value: internalEditor_0.actors.relayActor, children: t5 }), $[18] = internalEditor_0.actors.relayActor, $[19] = t5, $[20] = t6) : t6 = $[20];
  let t7;
  $[21] !== internalEditor_0.actors.editorActor || $[22] !== t6 ? (t7 = /* @__PURE__ */ jsx(EditorActorContext.Provider, { value: internalEditor_0.actors.editorActor, children: t6 }), $[21] = internalEditor_0.actors.editorActor, $[22] = t6, $[23] = t7) : t7 = $[23];
  let t8;
  return $[24] !== internalEditor_0.editor || $[25] !== t7 ? (t8 = /* @__PURE__ */ jsx(EditorContext$1.Provider, { value: internalEditor_0.editor, children: t7 }), $[24] = internalEditor_0.editor, $[25] = t7, $[26] = t8) : t8 = $[26], t8;
}
